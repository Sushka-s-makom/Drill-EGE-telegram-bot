...
Запуск:
    python pregenerate_cards.py                   # карточки с условием
    python pregenerate_cards.py --solutions        # карточки с решением/ответом
    python pregenerate_cards.py --workers 8        # число потоков (по умолч. 6)
    python pregenerate_cards.py --retry            # только задания с ошибками
    python pregenerate_cards.py --force            # перегенерировать всё
...
CARDS_SOL_DIR = Path(__file__).parent / "cards_solution"
FAILED_LOG    = Path(__file__).parent / "cards_failed.txt"
# ─────────────────────────────────────────────
...
def run(question_ids: list[int], out_dir: Path,
    ...
    if failed:
        print(f"\n  Упавшие → {FAILED_LOG}")
        print(f"  Повторить: python pregenerate_cards.py --retry --solutions")
